...
fn test_sub_babybear_metal_fe() -> Result<(), MetalError> {
    ...
    let expected_output = vec![FE::from(1), FE::from(2013265920)]; // 4 - 3 = 1
...
fn test_mul_babybear_metal_fe() -> Result<(), MetalError> {
    ...
    // 2) Usamos varios pares de valores:
    //    lhs = [0], rhs = [3] => resultado = [0].
    let lhs = vec![FE::from(3), FE::from(2013265921), FE::from(2013265919)];
    let rhs = vec![FE::from(4), FE::from(3), FE::from(3)];
    let expected_output = vec![FE::from(12), FE::zero(), FE::from(2013265915)];
...
fn test_pow_babybear_metal_fe() -> Result<(), MetalError> {
    ...
    let base = vec![FE::from(2), FE::from(3), FE::from(2)];
    let exponent = vec![3u32, 2u32, 2013265920u32];
    let expected_output = vec![FE::from(8), FE::from(9), FE::one()];
...
fn test_mul_by_inv_babybear_metal_fe() -> Result<(), MetalError> {
    ...
    let input = vec![FE::from(2), FE::from(5)];
    let expected_output = vec![FE::one(), FE::one()];
